...
// Read the list of bad lessons
const badLessons = JSON.parse(fs.readFileSync('./bad-template-lessons.json', 'utf8'));

console.log(`Transforming ${badLessons.length} lessons to standard template...\n`);

// Read a good template for reference
const goodTemplatePath = './genrok-app/public/lessons/biz-infinite-flywheel/lesson.html';
const goodTemplate = fs.readFileSync(goodTemplatePath, 'utf8');

// Extract template parts from good template
function extractTemplateParts(goodTemplate) {
  // Head section up to <script type="text/babel">
  const headEnd = goodTemplate.indexOf('<script type="text/babel">');
  const head = goodTemplate.substring(0, headEnd);

  // Components section (from WelcomeSlide to before LessonApp)
  const welcomeStart = goodTemplate.indexOf('const WelcomeSlide');
  const lessonAppStart = goodTemplate.indexOf('const LessonApp');
  const components = goodTemplate.substring(welcomeStart, lessonAppStart);

  // LessonApp and footer
  const lessonApp = goodTemplate.substring(lessonAppStart);

  return { head, components, lessonApp };
}

// Extract data from bad lesson
function extractLessonData(content) {
  // Extract LESSON_CONFIG
  const configMatch = content.match(/const LESSON_CONFIG\s*=\s*(\{[\s\S]*?\});/);
  ...
  if (configMatch) {
    try {
      ...
      const configStr = configMatch[1]
        ...
        .replace(/,\s*}/g, '}')
        .replace(/,\s*]/g, ']');
      lessonConfig = JSON.parse(configStr);
    } catch (e) {
      ...
      lessonConfig = {
        ...
        title: titleMatch ? titleMatch[1] : 'Unknown Lesson',
        duration: durationMatch ? durationMatch[1] : '5 min',
        accentColor: accentMatch ? accentMatch[1] : '#f97316'
      };
    }
  }

  // Extract accent color from CSS
  const accentMatch = content.match(/--accent:\s*([#\w]+)/);
  const accentColor = accentMatch ? accentMatch[1] : (lessonConfig?.accentColor || '#f97316');

  // Extract slides array - this is tricky because it can be multi-line
  // Try both 'slides' and 'SLIDES' (uppercase)
  let slidesMatch = content.match(/const slides\s*=\s*\[([\s\S]*?)\];(?=\s*(?:const|function|\/\/))/);
  if (!slidesMatch) {
    slidesMatch = content.match(/const SLIDES\s*=\s*\[([\s\S]*?)\];(?=\s*(?:const|function|\/\/))/);
  }
  let slidesArray = [];
  if (slidesMatch) {
    // Keep the raw slides string for now
    slidesArray = '[' + slidesMatch[1] + ']';
  }

  // Extract nextLesson if present
  const nextLessonMatch = content.match(/nextLesson:\s*\{\s*id:\s*['"]([^'"]+)['"]\s*,\s*title:\s*['"]([^'"]+)['"]\s*,\s*teaser:\s*['"]([^'"]+)['"]/);
  let nextLesson = null;
  if (nextLessonMatch) {
    nextLesson = {
      id: nextLessonMatch[1],
      title: nextLessonMatch[2],
      teaser: nextLessonMatch[3]
    };
  }
  ...
  return {
    config: lessonConfig,
    accentColor,
    slidesRaw: slidesMatch ? slidesMatch[0] : null,
    nextLesson
  };
}

// Generate standard template with lesson data
function generateStandardTemplate(lessonData, lessonId) {
  const { config, accentColor, slidesRaw, nextLesson } = lessonData;

  const title = config?.title || 'Lesson';
  const duration = config?.duration || '5 min';
  const id = config?.id || lessonId;

  // Calculate accent color variants
  const accentDark = adjustColor(accentColor, -20);
  const accentLight = adjustColor(accentColor, 20);
  const accentRgb = hexToRgb(accentColor);

  // Use the raw slides if available, otherwise create a basic placeholder
  const slides = slidesRaw || `const slides = [
      { type: 'welcome', learningGoals: ['Goal 1', 'Goal 2', 'Goal 3'] },
      { type: 'hook', headline: 'Hook headline', subtext: 'Hook subtext' },
      { type: 'content', title: 'Content Title', body: 'Content body text.' },
      { type: 'quiz', question: 'Quiz question?', options: ['A', 'B', 'C', 'D'], correctIndex: 0, feedback: { correct: 'Correct!', incorrect: 'Try again.' }},
      { type: 'completion', sources: [] }
    ];`;

  const nextLessonConfig = nextLesson ?
    `nextLesson: { id: '${nextLesson.id}', title: '${nextLesson.title}', teaser: '${nextLesson.teaser}' }` :
    `nextLesson: { id: 'next-lesson', title: 'Next Lesson', teaser: 'Continue your learning journey.' }`;
...
<head>
  ...
  <title>${title} | Quantum Scale</title>
  ...
  <style>
    :root { --accent: ${accentColor}; --accent-dark: ${accentDark}; --accent-light: ${accentLight}; --accent-rgb: ${accentRgb}; }
  ...
  <script type="text/babel">
    ...
    const CompletionSlide = ({ data }) => (
      <div className="h-full flex flex-col items-center justify-center text-center px-8 slide-scroll overflow-y-auto py-8">
        ...
        <motion.p initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="text-neutral-500 text-sm mb-5 max-w-md leading-relaxed">Great work! You've completed this lesson.</motion.p>
...
// Process each bad lesson
let transformed = 0;
let failed = [];

for (const lessonId of badLessons) {
  const lessonPath = `./genrok-app/public/lessons/${lessonId}/lesson.html`;

  if (!fs.existsSync(lessonPath)) {
    failed.push({ lessonId, reason: 'File not found' });
    continue;
  }
  ...
  try {
    ...
    if (!lessonData.slidesRaw) {
      failed.push({ lessonId, reason: 'Could not extract slides array' });
      continue;
    }

    const newContent = generateStandardTemplate(lessonData, lessonId);
    fs.writeFileSync(lessonPath, newContent);
    transformed++;
    console.log(`✓ ${lessonId}`);
  } catch (e) {
    failed.push({ lessonId, reason: e.message });
    console.log(`✗ ${lessonId}: ${e.message}`);
  }
}
...
if (failed.length > 0) {
  console.log(`\nFailed lessons:`);
  failed.forEach(({ lessonId, reason }) => {
