/**
 * Visual QA for lessons using Puppeteer
 * Screenshots saved to C:\Projects\Quantum-Scale\lesson-screenshots
 */
...
async function runVisualQA(lessonName) {
  console.log('='.repeat(60));
  console.log(`Visual QA for ${lessonName} Lesson`);
  console.log('='.repeat(60));
  console.log('');
  ...
  if (issues.length === 0) {
    console.log(`\n✓ ${lessonName} lesson passed visual QA!`);
  } else {
...
if (!lessonName) {
  console.error('Usage: node qa-lesson.js <lesson-name>');
  console.error('Example: node qa-lesson.js unique-mechanism');
  process.exit(1);
}

runVisualQA(lessonName).catch(console.error);
