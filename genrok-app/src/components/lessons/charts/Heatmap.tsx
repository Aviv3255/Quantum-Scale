 ...
 * Elite design with smooth cell animations
 */
  ...
  return (
    <div className={`${bgClass} p-8 rounded-2xl`}>
        ...
        <div className="inline-block">
          ...
          <div className="flex" style={{ marginLeft: labelWidth }}>
            {cols.map((col, i) => (
              <motion.div
                key={col}
                className={`flex items-center justify-center text-xs font-medium ${mutedClass}`}
          ...
          {rows.map((row, rowIndex) => (
            <div key={row} className="flex items-center">
              {/* Row label */}
              <motion.div
                className={`text-xs font-medium ${mutedClass} pr-3`}
      ...
      >
        ...
        />
        <span className={`text-xs ${mutedClass}`}>High</span>
      </motion.div>
    </div>
  );
}
