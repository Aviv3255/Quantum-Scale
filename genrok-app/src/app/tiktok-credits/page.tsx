...
const faqs = [
  {
    ...
    answer:
      'Credits unlock progressively as you reach spend milestones. Spend $2K and get $1K credit, spend $5K and get $3K total credits, spend $10K and get $6K total credits.',
  },
  {
    question: 'How long do I have to use the credits?',
    answer:
      'Credits are valid for 30 days from the date they are applied to your account. This gives you plenty of time to use them effectively.',
  },
  {
    question: 'Can existing TikTok advertisers apply?',
    answer:
      'Yes! Both new and existing TikTok advertisers can apply. If you have an existing account, we can help you migrate or create a new one for the partnership.',
  },
  {
    question: 'What types of businesses qualify?',
    answer:
      'eCommerce businesses selling physical or digital products are eligible. We work with Shopify, WooCommerce, and other major platforms.',
  },
];
    ...
    <DashboardLayout>
      <div className="page-wrapper">
        ...
        <header className="page-header">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
              ...
              <p>
                Scale faster with our exclusive TikTok partnership. Unlock your tier, launch
                instantly, and get priority support.
              </p>
            </div>
        ...
        <section className="mb-12">
          <div className="mb-8 text-center">
            ...
            <p className="text-[var(--text-muted)]">
              The more you spend, the more free credits you unlock.
            </p>
          </div>
          ...
          >
            {creditTiers.map((tier, index) => (
              <motion.div key={index} variants={itemVariants}>
                <div
                  className={`card relative text-center ${tier.popular ? 'scale-105 ring-2 ring-[var(--accent-gold)]' : ''}`}
                >
                  {tier.popular && (
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-[var(--primary)] px-4 py-1">
                      <span className="text-xs font-semibold text-white">MOST POPULAR</span>
                    </div>
                  )}

                  <div className="mb-6 rounded-xl bg-[var(--bg-secondary)] p-6">
                    <p className="mb-1 text-sm font-medium text-[var(--text-muted)]">Spend</p>
                    <p className="text-3xl font-bold text-[var(--text-primary)]">{tier.spend}</p>
                  </div>

                  <div className="mb-6">
                    <p className="mb-1 text-sm font-medium text-[var(--text-muted)]">Get</p>
                    <p className="text-5xl font-bold text-[var(--text-primary)]">{tier.credit}</p>
                    <p className="mt-1 text-sm text-[var(--text-muted)]">in free credits</p>
                  </div>
                  ...
                  <div className="flex items-center justify-center gap-2 text-green-600">
                    ...
                    <span className="font-semibold">
                      {Math.round(
                        (parseInt(tier.credit.replace(/\D/g, '')) /
                          parseInt(tier.spend.replace(/\D/g, ''))) *
                          100
                      )}
                      % bonus
                    </span>
        ...
        <section className="mb-12">
          <div className="mb-8 text-center">
            <h2 className="mb-2 text-xl font-semibold text-[var(--text-primary)]">
              Why Use Our TikTok Partnership
            </h2>
            <p className="text-[var(--text-muted)]">
            ...
            {benefits.map((benefit, index) => (
              <motion.div key={index} variants={itemVariants}>
                <div className="card card-hover h-full">
                  <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-[var(--bg-secondary)]">
                    <benefit.icon
                      size={24}
        ...
        {/* How It Works */}
        <section className="card mb-12 bg-[var(--text-primary)] p-8">
            ...
            {steps.map((step, index) => (
              <div key={index} className="relative">
                {index < steps.length - 1 && (
                  <div className="bg-[var(--primary)]/30 absolute left-full top-8 hidden h-0.5 w-full md:block" />
                )}
                <div className="text-[var(--text-primary)]/30 mb-4 text-5xl font-bold">
                  {step.number}
                </div>
        ...
        {/* CTA */}
        <section className="card bg-[var(--primary)] p-8 text-center">
          <h2 className="mb-4 text-2xl font-bold text-white md:text-3xl">
            Ready to Scale with Free Ad Credits?
          </h2>
          <p className="mb-8 text-lg text-white/80">
            Join thousands of eCommerce brands already using our TikTok partnership to scale
            profitably.
          </p>
          <a
